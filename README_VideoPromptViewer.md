...
The original prompt stays in its original order, including spacing, line breaks, headings and markup. The current timed action and general directions use different colors. Both versions offer a Style dropdown next to Auto-scroll: Spotlight (serif text with a full-width warm highlight and timestamp gutter), Cards (sans-serif scene cards), Script (monospaced text with a blue highlight), and Classic (the original inline gold highlight). Spotlight is the default. The style selection is saved with the workflow; switching styles preserves playback position and prompt text. Auto-scroll follows playback and can be disabled. Time buttons sit below the seek bar at their actual positions; closely spaced labels use separate rows. Resize the node from its bottom-right corner to expand both columns and the prompt area.

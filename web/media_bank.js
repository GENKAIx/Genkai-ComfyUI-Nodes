/* Port of Fantastic MiniMax H3 PromptBuilder 1.7.1.
 * Copyright (c) 2026 Adudeguyman. MIT license: ../h3_media/LICENSE.
 * GENKAI additions: persistent slots, clipboard, themes and adjustable layout. */
...
layoutSheet.rel = "stylesheet";
layoutSheet.href = new URL("./media_layout.css", import.meta.url).href;
document.head.append(layoutSheet);
...
function responsiveSizing(node) {
  const widget = node._mmlWidget;
  if (!widget) return;
  if (node._mmlNativeCompute) widget.computeSize = node._mmlNativeCompute;
  else delete widget.computeSize;
  widget.options ||= {};
  widget.options.getMinHeight = () => 360;
  widget.options.getMaxHeight = () => Infinity;
  const root = node._mmlPanel?.root;
  if (root) { root.style.height = "100%"; root.style.minHeight = "360px"; }
  const element = widget.element || widget.inputEl;
  if (element && element !== root) { element.style.height = "100%"; element.style.minHeight = "360px"; }
}

document.addEventListener("paste", (e) => {
  const target = hoveredSlot;
  if (!target?.panel.root.isConnected || e.target?.closest?.("input,textarea,[contenteditable='true']")) return;
  const files = [...(e.clipboardData?.items || [])]
    .filter(item => item.kind === "file" && item.type.startsWith("image/"))
    .map(item => item.getAsFile()).filter(Boolean);
  if (!files.length) return;
  e.preventDefault(); e.stopImmediatePropagation();
  target.panel.add(files, target.slot);
}, true);

export const LOADER_NAME = "GenkaiMediaBank";
export const SPLITTER_NAME = "GenkaiReferenceSplitter";
...
const LOADER_PREF_KEY = "genkai.h3.loaderScale";
...
export function applyNodeSize(node, factor) {
  ...
  try {
    ...
    if (layoutPrefs(node).dynamicResize) responsiveSizing(node);
...
export function applyTextScale(panel, factor) {
  ...
  try {
    document.documentElement.style.setProperty("--gkh3mml-fs", String(f));
 ...
 *  rebuilt from the base dimensions with --gkh3mml-fs unset — a correct-sized
...
export function applyStoredScale(node, { force = false } = {}) {
  ...
  if (layoutPrefs(node).dynamicResize) { responsiveSizing(node); return; }
...
const CSS = `
.gkh3mml-panel{font-family:system-ui,sans-serif;color:#d7dbe2;font-size:calc(12px * var(--gkh3mml-fs, 1));
...
.gkh3mml-cols{flex:1;min-height:0;display:grid;grid-template-columns:1fr 1fr;gap:9px;}
.gkh3mml-col{display:flex;flex-direction:column;gap:5px;min-width:0;}
.gkh3mml-modal .gkh3mml-panel{border:0;height:100%;min-height:0;}
.gkh3mml-overlay{position:fixed;inset:0;z-index:10040;background:rgba(8,10,14,.62);
  display:flex;align-items:center;justify-content:center;}
.gkh3mml-modal{width:min(1140px,96vw);height:min(780px,92vh);background:#191c22;
...
.gkh3mml-modalhead{display:flex;align-items:center;gap:10px;padding:9px 13px;
  background:#1e222a;border-bottom:1px solid #2a2f3a;font-size:calc(13px * var(--gkh3mml-fs, 1));
  font-weight:500;color:#d7dbe2;font-family:system-ui,sans-serif;}
.gkh3mml-modalhead button{margin-left:auto;background:none;border:0;color:#8a93a3;
  font-size:calc(17px * var(--gkh3mml-fs, 1));cursor:pointer;}
.gkh3mml-modalhead button:hover{color:#fff;}
/* Draft-bound media modal: same panel, different target, so it has to look
   different. Teal matches the editor's draft chrome. */
.gkh3mml-modal.draft{border-color:#3fb2a8;box-shadow:0 24px 64px rgba(0,0,0,.55),
  0 0 0 1px #3fb2a8;}
.gkh3mml-modal.draft .gkh3mml-modalhead{background:#15242a;border-bottom-color:#3fb2a8;}
.gkh3mml-draftbadge{background:#3fb2a8;color:#06211f;font-weight:700;
  border-radius:5px;padding:1px 7px;letter-spacing:.06em;margin-right:2px;
  font-size:calc(10px * var(--gkh3mml-fs, 1));font-family:system-ui,sans-serif;}
.gkh3mml-draftnote{background:#15242a;border-bottom:1px solid #24343a;
  color:#bfe0dc;padding:6px 13px;line-height:1.45;
  font-size:calc(11px * var(--gkh3mml-fs, 1));font-family:system-ui,sans-serif;}
.gkh3mml-modalbody{flex:1;min-height:0;padding:8px;overflow:auto;}
.gkh3mml-panel.drop{border-color:#6f86b8;background:#1d2330;}
.gkh3mml-top{display:flex;align-items:center;gap:8px;flex:0 0 auto;min-width:0;}
.gkh3mml-top .gkh3mml-btn,.gkh3mml-top .gkh3mml-count{flex:0 0 auto;white-space:nowrap;}
.gkh3mml-btn{background:#2b3140;border:1px solid #3a4252;color:#d7dbe2;border-radius:6px;
  padding:4px 10px;font-size:calc(11px * var(--gkh3mml-fs, 1));cursor:pointer;}
.gkh3mml-btn:hover{background:#333b4d;}
.gkh3mml-presetrow{flex:0 0 auto;display:flex;align-items:center;gap:5px;
  min-width:0;flex-wrap:nowrap;}
.gkh3mml-presetrow .gkh3mml-btn{flex:0 0 auto;white-space:nowrap;}
.gkh3mml-presetlbl{flex:0 0 auto;white-space:nowrap;
  font-size:calc(10px * var(--gkh3mml-fs, 1));text-transform:uppercase;letter-spacing:.07em;
  color:#6b7484;}
.gkh3mml-btn.gkh3mml-sm{padding:3px 9px;font-size:calc(10px * var(--gkh3mml-fs, 1));}
.gkh3mml-btn.gkh3mml-danger{border-color:#7a3a3a;color:#f0a0a0;}
.gkh3mml-btn.gkh3mml-danger:hover{background:#3a2020;}
.gkh3mml-presetname{flex:1;min-width:0;background:#12151b;color:#dde2ea;
  border:1px solid #4a5568;border-radius:6px;padding:3px 7px;font-size:calc(11px * var(--gkh3mml-fs, 1));
  font-family:system-ui,sans-serif;}
.gkh3mml-presetname:focus{outline:none;border-color:#6f86b8;}
.gkh3mml-presetwarn{flex:1;min-width:0;font-size:calc(10px * var(--gkh3mml-fs, 1));color:#e0a94c;overflow:hidden;
  text-overflow:ellipsis;white-space:nowrap;}
.gkh3mml-topspace{flex:1;}
.gkh3mml-scalewrap{position:relative;display:inline-block;}
...
.gkh3mml-scalemenu{--gkh3mml-fs:1;position:absolute;right:0;top:100%;margin-top:6px;
...
.gkh3mml-scalemenu.on{display:block;}
.gkh3mml-scalerow{display:flex;align-items:center;gap:8px;padding:5px 4px;}
.gkh3mml-scalelabel{font-size:calc(10px * var(--gkh3mml-fs, 1));color:#8a93a3;
  width:62px;flex:0 0 auto;white-space:nowrap;}
.gkh3mml-scalerange{flex:1;min-width:0;}
.gkh3mml-scaleval{font-size:calc(10px * var(--gkh3mml-fs, 1));color:#d7dbe2;
...
.gkh3mml-scaleval:focus{outline:none;border-color:#4a5568;}
.gkh3mml-scalepct{font-size:calc(10px * var(--gkh3mml-fs, 1));color:#6b7484;
  flex:0 0 auto;margin-left:-2px;}
.gkh3mml-scalefoot{display:flex;align-items:center;gap:6px;
  border-top:1px solid #2a2f3a;margin-top:6px;padding-top:7px;
  font-size:calc(9px * var(--gkh3mml-fs, 1));color:#6b7484;}
.gkh3mml-scalefoot span{flex:1;min-width:0;line-height:1.25;}
.gkh3mml-count{font-size:calc(10px * var(--gkh3mml-fs, 1));color:#8a93a3;font-family:ui-monospace,monospace;}
.gkh3mml-count.over{color:#f07070;}
.gkh3mml-msg{flex:0 0 auto;font-size:calc(10px * var(--gkh3mml-fs, 1));min-height:12px;color:#e0a94c;overflow:hidden;
  text-overflow:ellipsis;white-space:nowrap;}
.gkh3mml-msg.err{color:#f07070;}
.gkh3mml-sec{flex:0 0 auto;display:flex;align-items:center;font-size:calc(10px * var(--gkh3mml-fs, 1));
  text-transform:uppercase;letter-spacing:.07em;color:#6b7484;}
.gkh3mml-sec span{margin-left:auto;text-transform:none;letter-spacing:0;color:#5c6472;
  font-family:ui-monospace,monospace;}

.gkh3mml-pics{flex:1;min-height:0;display:grid;
...
.gkh3mml-vids{flex:46 1 auto;min-height:148px;display:grid;
  grid-template-rows:repeat(3,1fr);gap:5px;
  grid-template-columns:minmax(0,1fr);}
.gkh3mml-spacer{flex:1;min-height:0;}
.gkh3mml-auds{flex:38 1 auto;min-height:124px;display:grid;
  grid-template-rows:repeat(3,1fr);gap:5px;
  grid-template-columns:minmax(0,1fr);}

.gkh3mml-slot{border:1px dashed #2b313d;border-radius:6px;background:#141820;
  display:flex;align-items:center;justify-content:center;gap:5px;color:#4d5563;
  font-size:calc(10px * var(--gkh3mml-fs, 1));cursor:pointer;overflow:hidden;min-width:0;min-height:0;}
.gkh3mml-slot:hover{border-color:#59637a;color:#8a93a3;}
.gkh3mml-slot.hot{border-color:#6f86b8;background:#1b2230;color:#9db4dc;}
.gkh3mml-slot.filled{border-style:solid;border-color:#2e3440;background:#12151b;cursor:default;
  display:block;position:relative;min-width:0;min-height:0;overflow:hidden;}
.gkh3mml-slot.filled.pic{border-color:#6d5527;}
.gkh3mml-slot.filled.vid{border-color:#255c6b;}
.gkh3mml-slot.filled.aud{border-color:#4c3d6e;}
.gkh3mml-slot.dragging{opacity:.35;}
.gkh3mml-slot.over{outline:1px solid #6f86b8;outline-offset:1px;}
...
.gkh3mml-cropfit{position:absolute;inset:0;pointer-events:none;}
...
.gkh3mml-pic.turned{width:auto;height:auto;max-width:none;max-height:none;
  inset:0;margin:auto;}
.gkh3mml-cropbox{position:absolute;line-height:0;}
.gkh3mml-cropmark{position:absolute;border:1px solid rgba(76,195,224,.9);
  box-shadow:0 0 0 2000px rgba(6,8,12,.55);pointer-events:none;z-index:1;}
.gkh3mml-dims.cut{color:#9fe3f5;}
.gkh3mml-dims{position:absolute;right:3px;top:3px;padding:1px 4px;border-radius:4px;
  background:rgba(8,10,14,.85);color:#dfe4ec;font-size:calc(8px * var(--gkh3mml-fs, 1));line-height:1.2;
  font-family:ui-monospace,monospace;pointer-events:none;letter-spacing:0;
  text-shadow:0 1px 2px rgba(0,0,0,.9);z-index:2;}
.gkh3mml-dims:empty{display:none;}
.gkh3mml-lightdims{font-size:calc(10px * var(--gkh3mml-fs, 1));color:#8a93a3;font-family:ui-monospace,monospace;}
.gkh3mml-pic{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;
  display:block;cursor:zoom-in;background:#0d1015;}
.gkh3mml-picbar{position:absolute;left:0;right:0;bottom:0;display:flex;align-items:center;
...
.gkh3mml-picbar .gkh3mml-tag{flex:1 1 auto;min-width:0;overflow:hidden;
  text-overflow:ellipsis;}
.gkh3mml-picbar .gkh3mml-power,
.gkh3mml-picbar .gkh3mml-trimbtn,
.gkh3mml-picbar .gkh3mml-drag,
.gkh3mml-picbar .gkh3mml-x{flex:0 0 auto;}
.gkh3mml-picbar .gkh3mml-trimbtn{font-size:calc(12px * var(--gkh3mml-fs, 1));}
.gkh3mml-tag{font-family:ui-monospace,monospace;font-size:calc(9px * var(--gkh3mml-fs, 1));white-space:nowrap;}
.gkh3mml-tag.pic{color:#e0a94c;} .gkh3mml-tag.vid{color:#4cc3e0;} .gkh3mml-tag.aud{color:#b48ce8;}
.gkh3mml-x{cursor:pointer;color:#7a8393;font-size:calc(11px * var(--gkh3mml-fs, 1));line-height:1;}
.gkh3mml-x:hover{color:#e05a5a;}

.gkh3mml-row{display:flex;align-items:center;gap:6px;padding:0 6px;height:100%;
  box-sizing:border-box;min-width:0;overflow:hidden;}
.gkh3mml-vthumb{width:60px;height:34px;min-width:60px;max-width:60px;border-radius:4px;
  object-fit:contain;background:#0d1015;flex-shrink:0;cursor:zoom-in;}
.gkh3mml-meta{min-width:0;flex:1;}
.gkh3mml-name{font-size:calc(9px * var(--gkh3mml-fs, 1));color:#6b7484;overflow:hidden;text-overflow:ellipsis;
  white-space:nowrap;}
.gkh3mml-play{width:20px;height:20px;border-radius:50%;border:1px solid #3a4252;background:#20242d;
  color:#c9cfda;font-size:calc(9px * var(--gkh3mml-fs, 1));line-height:1;cursor:pointer;flex-shrink:0;
  display:flex;align-items:center;justify-content:center;padding:0;}
.gkh3mml-play:hover{border-color:#59637a;}
.gkh3mml-bar{flex:1;height:3px;background:#2a2f3a;border-radius:2px;min-width:16px;
  cursor:pointer;position:relative;}
.gkh3mml-bar i{position:absolute;left:0;top:0;bottom:0;background:#7d63b8;border-radius:2px;
  display:block;width:0;}
.gkh3mml-time{font-size:calc(9px * var(--gkh3mml-fs, 1));color:#6b7484;font-family:ui-monospace,monospace;flex-shrink:0;}
.gkh3mml-seg{display:inline-flex;border:1px solid #2e3440;border-radius:4px;overflow:hidden;
  flex-shrink:0;}
.gkh3mml-seg button{background:none;border:0;color:#6b7484;font-size:calc(9px * var(--gkh3mml-fs, 1));padding:1px 5px;
  cursor:pointer;}
.gkh3mml-seg button.on{background:#3a2f56;color:#e2d6f8;}
.gkh3mml-power{cursor:pointer;color:#4d5563;font-size:calc(11px * var(--gkh3mml-fs, 1));line-height:1;flex-shrink:0;
  user-select:none;}
.gkh3mml-power.on{color:#7ec87e;}
.gkh3mml-power:hover{color:#a8e6a8;}
.gkh3mml-slot.filled.off{opacity:.42;border-style:dashed;}
.gkh3mml-slot.filled.off .gkh3mml-power{opacity:1;color:#6b7484;}
.gkh3mml-slot.filled.off:hover{opacity:.7;}
.gkh3mml-segstack{display:flex;flex-direction:column;align-items:center;gap:2px;
  flex-shrink:0;}
.gkh3mml-segtag{font-size:calc(9px * var(--gkh3mml-fs, 1));}
.gkh3mml-trimok{border-color:#3e5240;color:#7ec87e;}
.gkh3mml-trimbtn{cursor:pointer;color:#e0a94c;opacity:.65;font-size:calc(15px * var(--gkh3mml-fs, 1));line-height:1;
  flex-shrink:0;user-select:none;}
.gkh3mml-trimbtn:hover{opacity:1;}
.gkh3mml-trimbtn.on{opacity:1;text-shadow:0 0 6px rgba(224,169,76,.55);}
.gkh3mml-tmover{position:fixed;inset:0;background:rgba(8,10,14,.72);z-index:10050;
  display:flex;align-items:center;justify-content:center;}
.gkh3mml-tmmodal{width:min(640px,92vw);background:#191c22;border:1px solid #303642;
...
.gkh3mml-tmhead{display:flex;align-items:center;gap:8px;padding:8px 12px;
  border-bottom:1px solid #2a2f3a;background:#1b1f27;}
.gkh3mml-tmtitle{flex:1;min-width:0;font-size:calc(12px * var(--gkh3mml-fs, 1));color:#dde2ea;overflow:hidden;
  text-overflow:ellipsis;white-space:nowrap;}
...
.gkh3mml-tmstage{position:relative;background:#000;line-height:0;overflow:hidden;
  display:flex;align-items:center;justify-content:center;}
.gkh3mml-tmvideo{width:100%;max-height:340px;object-fit:contain;display:block;}
/* Turned: sizeMedia() sets an explicit width, so the percentage rules that
   assume an upright box must stand down. */
.gkh3mml-tmvideo.turned{width:auto;max-height:none;}
.gkh3mml-tmcropwrap{position:absolute;inset:0;}

.gkh3mml-tmcrop{position:absolute;border:1.5px dashed #4cc3e0;cursor:move;
  background:
...
.gkh3mml-tmcrop.locked{cursor:default;border-style:solid;
  border-color:rgba(76,195,224,.85);background:none;}
.gkh3mml-tmcrop.locked .gkh3mml-tmcorner{display:none;}
.gkh3mml-tmcorner{position:absolute;width:11px;height:11px;background:#4cc3e0;
  border-radius:2px;}
.gkh3mml-tmcorner.nw{left:-6px;top:-6px;cursor:nwse-resize;}
.gkh3mml-tmcorner.ne{right:-6px;top:-6px;cursor:nesw-resize;}
.gkh3mml-tmcorner.sw{left:-6px;bottom:-6px;cursor:nesw-resize;}
.gkh3mml-tmcorner.se{right:-6px;bottom:-6px;cursor:nwse-resize;}
.gkh3mml-tmcropbar{display:flex;align-items:center;gap:6px;}
.gkh3mml-tmcropinfo{font-size:calc(10px * var(--gkh3mml-fs, 1));color:#8a93a3;font-family:ui-monospace,monospace;
  white-space:nowrap;}
.gkh3mml-tmcropinfo.changed{color:#4cc3e0;}
.gkh3mml-tmlock{font-size:calc(11px * var(--gkh3mml-fs, 1));color:#b9cdef;border:1px solid #4d6ea6;border-radius:6px;
  padding:3px 8px;white-space:nowrap;}
.gkh3mml-tmaspect{background:#12151b;color:#c9cfda;border:1px solid #2e3440;
  border-radius:6px;padding:2px 5px;font-size:calc(11px * var(--gkh3mml-fs, 1));}
.gkh3mml-btn.on{background:#173642;border-color:#4cc3e0;color:#9fe3f5;}
.gkh3mml-tmtimeline{position:relative;padding:8px 14px 4px;}
.gkh3mml-tmwave{display:block;width:100%;height:46px;margin-bottom:2px;}
.gkh3mml-tmruler{position:relative;height:16px;}
.gkh3mml-tmtick{position:absolute;transform:translateX(-50%);font-size:calc(9px * var(--gkh3mml-fs, 1));
  color:#6b7484;}
.gkh3mml-tmtick::before{content:"";position:absolute;left:50%;top:-3px;width:1px;
  height:3px;background:#3a4252;}
.gkh3mml-tmbar{position:relative;height:20px;background:#12151b;border-radius:5px;
  margin:2px 0 6px;cursor:pointer;}
.gkh3mml-tmsel{position:absolute;top:0;bottom:0;background:#1f6f96;border-radius:5px;}
.gkh3mml-tmhandle{position:absolute;top:-3px;bottom:-3px;width:9px;background:#4cc3e0;
  border-radius:3px;transform:translateX(-50%);cursor:ew-resize;z-index:2;}
.gkh3mml-tmhandle:hover{background:#7fd8ee;box-shadow:0 0 6px rgba(76,195,224,.7);}
.gkh3mml-tmplayhead{position:absolute;top:-5px;bottom:-5px;width:2px;
...
.gkh3mml-tmplayhead::before{content:"";position:absolute;left:50%;top:-4px;
  width:0;height:0;transform:translateX(-50%);
  border-left:4px solid transparent;border-right:4px solid transparent;
  border-top:5px solid #ffb84d;}
.gkh3mml-tmnow{display:flex;gap:5px;align-items:center;height:14px;
  font-size:calc(9px * var(--gkh3mml-fs, 1));color:#8a6a33;text-transform:uppercase;letter-spacing:.06em;}
.gkh3mml-tmplaytime{color:#ffb84d;font-family:ui-monospace,monospace;
  text-transform:none;letter-spacing:0;font-size:calc(10px * var(--gkh3mml-fs, 1));}
.gkh3mml-tmfoot{display:flex;align-items:center;gap:5px;padding:8px 12px 0;
  flex-wrap:wrap;}
.gkh3mml-tmfoot.act{padding:8px 12px 4px;border-top:1px solid #23272f;margin-top:8px;}
.gkh3mml-tmgap{width:8px;}
.gkh3mml-tmspace{flex:1;}
.gkh3mml-tmnum{width:52px;background:#12151b;color:#dde2ea;border:1px solid #2e3440;
  border-radius:6px;padding:3px 6px;font-size:calc(11px * var(--gkh3mml-fs, 1));text-align:right;
  font-family:ui-monospace,monospace;}
.gkh3mml-tmnum:focus{outline:none;border-color:#4cc3e0;}
.gkh3mml-tmdash{color:#5c6472;font-size:calc(11px * var(--gkh3mml-fs, 1));}
.gkh3mml-tmoutside{font-size:calc(10px * var(--gkh3mml-fs, 1));color:#f07070;white-space:nowrap;overflow:hidden;
  text-overflow:ellipsis;text-transform:none;letter-spacing:0;}
.gkh3mml-tmplayhead.out{background:#f07070;
  box-shadow:0 0 0 1px rgba(0,0,0,.65), 0 0 7px rgba(240,112,112,.85);}
.gkh3mml-tmplayhead.out::before{border-top-color:#f07070;}
.gkh3mml-tmnote{padding:2px 12px 6px;font-size:calc(10px * var(--gkh3mml-fs, 1));color:#8a93a3;line-height:1.4;}
.gkh3mml-tmnote.bad{color:#f07070;}
.gkh3mml-tmnote:empty{display:none;}
.gkh3mml-tmkeys{padding:0 12px 10px;font-size:calc(10px * var(--gkh3mml-fs, 1));color:#5c6472;}
.gkh3mml-tmreadout{font-size:calc(11px * var(--gkh3mml-fs, 1));color:#8a93a3;font-family:ui-monospace,monospace;}
.gkh3mml-tmreadout.bad{color:#f07070;}
.gkh3mml-btn.primary{background:#1f4f7d;border-color:#3d7fbf;color:#dbeafe;}
.gkh3mml-trimrow{display:flex;align-items:center;flex-wrap:nowrap;gap:3px;
  padding:0 5px;height:100%;overflow:hidden;}
.gkh3mml-trimlbl{font-size:calc(9px * var(--gkh3mml-fs, 1));text-transform:uppercase;letter-spacing:.07em;
  color:#6b7484;}
.gkh3mml-triminput{width:38px;background:#12151b;color:#dde2ea;
  border:1px solid #2e3440;border-radius:5px;padding:2px 6px;font-size:calc(11px * var(--gkh3mml-fs, 1));}
.gkh3mml-triminput:focus{outline:none;border-color:#4a5568;}
.gkh3mml-trimdash{color:#6b7484;}
.gkh3mml-trimof{font-size:calc(10px * var(--gkh3mml-fs, 1));color:#6b7484;}
.gkh3mml-trimerr{flex-basis:100%;font-size:calc(10px * var(--gkh3mml-fs, 1));color:#f07070;}
.gkh3mml-trimerr:empty{display:none;}
.gkh3mml-drag{cursor:grab;color:#4d5563;font-size:calc(10px * var(--gkh3mml-fs, 1));user-select:none;flex-shrink:0;}

.gkh3mml-order{flex:0 0 auto;background:#1a2230;border:1px solid #2b3a52;border-radius:6px;
  padding:4px 7px;height:42px;box-sizing:border-box;overflow:hidden;}
.gkh3mml-order b{display:block;font-size:calc(9px * var(--gkh3mml-fs, 1));text-transform:uppercase;letter-spacing:.07em;
  color:#6f86b8;font-weight:500;margin-bottom:1px;}
.gkh3mml-order div{font-family:ui-monospace,monospace;font-size:calc(9px * var(--gkh3mml-fs, 1));color:#9db4dc;
  line-height:1.35;overflow:hidden;}

.gkh3mml-light{position:fixed;inset:0;z-index:10050;background:rgba(8,10,14,.75);
  display:flex;align-items:center;justify-content:center;}
.gkh3mml-lightbox{max-width:80vw;max-height:80vh;background:#1e222a;border:1px solid #3a4252;
  border-radius:10px;overflow:hidden;padding:8px;}
.gkh3mml-lightbox img,.gkh3mml-lightbox video{max-width:76vw;max-height:68vh;display:block;}
.gkh3mml-lightcap{display:flex;align-items:center;gap:8px;padding-top:6px;font-size:calc(11px * var(--gkh3mml-fs, 1));
  color:#8a93a3;}
.gkh3mml-helpbtn{margin-left:5px;width:13px;height:13px;line-height:1;padding:0;
  border-radius:50%;border:1px solid #3a4252;background:#20242d;color:#8a93a3;
  font-size:calc(9px * var(--gkh3mml-fs, 1));cursor:pointer;font-family:system-ui,sans-serif;}
.gkh3mml-helpbtn:hover{border-color:#6f86b8;color:#c9cfda;}
.gkh3mml-help{position:fixed;z-index:10055;width:370px;max-height:min(560px,88vh);
  ...
  font-family:system-ui,sans-serif;}
.gkh3mml-helphead{display:flex;align-items:center;padding:7px 10px;background:#232833;
  border-bottom:1px solid #2a2f3a;font-size:calc(11px * var(--gkh3mml-fs, 1));text-transform:uppercase;
  letter-spacing:.07em;color:#8a93a3;}
.gkh3mml-helphead button{margin-left:auto;background:none;border:0;color:#6b7484;
  font-size:calc(13px * var(--gkh3mml-fs, 1));cursor:pointer;line-height:1;}
.gkh3mml-helphead button:hover{color:#fff;}
.gkh3mml-helpbody{overflow:auto;padding:9px 10px;}
.gkh3mml-helpbody p{margin:0;font-size:calc(11px * var(--gkh3mml-fs, 1));line-height:1.55;color:#aab2c0;}
.gkh3mml-helprow{display:flex;gap:8px;margin-bottom:9px;}
.gkh3mml-helpmode{flex:0 0 auto;font-family:ui-monospace,monospace;font-size:calc(10px * var(--gkh3mml-fs, 1));
  border-radius:9px;padding:1px 7px;height:16px;line-height:14px;
  border:1px solid #363d4a;background:#20242d;color:#8a93a3;}
.gkh3mml-helpmode.paired{border-color:#7d63b8;background:#3a2f56;color:#e2d6f8;}
.gkh3mml-helpmode.alone{border-color:#2c6f81;background:#1d3a44;color:#a5e2f0;}
.gkh3mml-helpsub{font-size:calc(10px * var(--gkh3mml-fs, 1));text-transform:uppercase;letter-spacing:.07em;
  color:#6b7484;margin:12px 0 6px;padding-top:8px;border-top:1px solid #2a2f3a;}
.gkh3mml-wirerow{display:flex;align-items:center;gap:5px;flex-wrap:wrap;margin-bottom:6px;}
.gkh3mml-wirerow code{font-family:ui-monospace,monospace;font-size:calc(10px * var(--gkh3mml-fs, 1));color:#9db4dc;
  background:#181c24;border-radius:4px;padding:1px 5px;}
.gkh3mml-arrow{color:#5c6472;font-size:calc(10px * var(--gkh3mml-fs, 1));}
.gkh3mml-tags{font-family:ui-monospace,monospace;font-size:calc(9px * var(--gkh3mml-fs, 1));color:#6b7484;
  flex-basis:100%;padding-left:2px;}
.gkh3mml-helpnote{margin-top:10px !important;padding-top:9px;
  border-top:1px solid #2a2f3a;color:#8a93a3 !important;}
.gkh3mml-toast{position:fixed;bottom:24px;left:50%;transform:translateX(-50%);z-index:10060;
  background:#2b3140;color:#fff;border:1px solid #4a5568;border-radius:8px;
  padding:8px 16px;font-size:calc(13px * var(--gkh3mml-fs, 1));font-family:system-ui,sans-serif;}
...
.gkh3mml-presetwrap{position:relative;flex:1 1 0;min-width:0;display:flex;}
.gkh3mml-presetbtn{flex:1 1 0;min-width:0;text-align:left;background:#12151b;color:#c9cfda;
  border:1px solid #2e3440;border-radius:6px;padding:3px 22px 3px 7px;
  font-size:calc(11px * var(--gkh3mml-fs, 1));font-family:system-ui,sans-serif;cursor:pointer;
  position:relative;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
.gkh3mml-presetbtn:after{content:"\\25be";position:absolute;right:7px;top:50%;
  transform:translateY(-50%);color:#6b7484;}
.gkh3mml-presetbtn:hover,.gkh3mml-presetbtn.on{border-color:#4a5568;}
.gkh3mml-presetbtn:focus{outline:none;border-color:#4a5568;}
.gkh3mml-presetmenu{display:none;position:absolute;left:0;right:0;top:100%;margin-top:4px;
...
.gkh3mml-presetmenu.on{display:block;}
.gkh3mml-presetitem{padding:4px 8px;font-size:calc(11px * var(--gkh3mml-fs, 1));color:#c9cfda;
  font-family:system-ui,sans-serif;cursor:pointer;overflow:hidden;
  text-overflow:ellipsis;white-space:nowrap;}
.gkh3mml-presetitem{display:flex;align-items:baseline;gap:6px;}
.gkh3mml-presetitemname{flex:1 1 auto;overflow:hidden;text-overflow:ellipsis;}
.gkh3mml-presetitemn{flex:0 0 auto;color:#5c6472;
  font-size:calc(9px * var(--gkh3mml-fs, 1));}
.gkh3mml-presetitem:hover{background:#232a35;}
.gkh3mml-presetcatbtn{flex:0 0 auto;background:none;border:0;color:#4a5568;
  cursor:pointer;padding:0 2px;font-size:calc(10px * var(--gkh3mml-fs, 1));}
.gkh3mml-presetitem:hover .gkh3mml-presetcatbtn{color:#8a93a3;}
.gkh3mml-presetcatbtn:hover{color:#dde2ea;}
.gkh3mml-presetitem.editing{background:#1d2430;gap:5px;flex-wrap:nowrap;
  align-items:center;}
...
.gkh3mml-presetitem.editing .gkh3mml-presetitemname{flex:0 1 auto;max-width:38%;}
.gkh3mml-presetitem.editing .gkh3mml-presetcat{flex:0 1 150px;min-width:0;}
.gkh3mml-presetitem.editing .gkh3mml-presetcatnew{flex:1 1 90px;min-width:0;}
.gkh3mml-presetitem.editing .gkh3mml-btn{flex:0 0 auto;}
.gkh3mml-presethead{padding:5px 8px 2px;color:#6b7484;letter-spacing:.05em;
  text-transform:uppercase;font-size:calc(9px * var(--gkh3mml-fs, 1));
  font-family:system-ui,sans-serif;position:sticky;top:0;background:#161a21;}
.gkh3mml-presetlist{max-height:220px;overflow:auto;}
/* The picker's bar mirrors the prompt library's: search, category select,
   rename. Same job, same shape. */
.gkh3mml-presetbar{display:flex;gap:5px;align-items:center;padding:6px 7px;
  border-bottom:1px solid #2e3440;background:#12151b;}
.gkh3mml-presetfilter{flex:1 1 auto;min-width:0;box-sizing:border-box;
  background:#191c22;color:#dde2ea;border:1px solid #2e3440;border-radius:6px;
  padding:4px 7px;font-size:calc(11px * var(--gkh3mml-fs, 1));
  font-family:system-ui,sans-serif;}
.gkh3mml-presetfilter:focus{outline:none;border-color:#4a5568;}
.gkh3mml-presetcatfilter{flex:0 1 130px;min-width:0;background:#191c22;
  color:#c9cfda;border:1px solid #2e3440;border-radius:6px;padding:4px 5px;
  font-size:calc(11px * var(--gkh3mml-fs, 1));font-family:system-ui,sans-serif;}
.gkh3mml-presetcatfilter:focus{outline:none;border-color:#4a5568;}
.gkh3mml-presetcatedit{flex:0 0 auto;}
.gkh3mml-presetcatedit.on{border-color:#4a5568;color:#dde2ea;}
.gkh3mml-presetrenamerow{display:flex;gap:5px;align-items:center;}
.gkh3mml-presetrenamerow:not(:empty){padding:6px 7px;
  border-bottom:1px solid #2e3440;background:#151920;}
.gkh3mml-presetrenamerow .gkh3mml-presetcatnew{flex:1 1 auto;min-width:0;display:block;}
.gkh3mml-presetcat,.gkh3mml-presetcatnew{background:#12151b;color:#c9cfda;
  border:1px solid #2e3440;border-radius:6px;padding:3px 6px;flex:0 0 auto;
  max-width:150px;font-size:calc(11px * var(--gkh3mml-fs, 1));
  font-family:system-ui,sans-serif;}
.gkh3mml-presetcat:focus,.gkh3mml-presetcatnew:focus{outline:none;border-color:#4a5568;}
.gkh3mml-presetitem.on{color:#dde2ea;background:#1d2430;}
.gkh3mml-presetempty{padding:4px 8px;font-size:calc(10px * var(--gkh3mml-fs, 1));color:#6b7484;
  font-family:system-ui,sans-serif;}
`;
...
class TrimModal {
  ...
  constructor(panel, item, opts = {}) {
    ...
    scaleOverlay(this.panel?.node, [
      [this.overlay.querySelector(".gkh3mml-tmmodal"), 640, 0],
    ]);
  ...
  buildMedia() {
    ...
    if (this.isStill) {
      this.media = el("img", { class: "gkh3mml-tmvideo", src: url,
    ...
    if (this.item.kind === "video") {
      this.media = el("video", { class: "gkh3mml-tmvideo", src: url,
    ...
    this.muteBtn = el("button", { class: "gkh3mml-btn gkh3mml-sm",
      title: "Mute the preview (M)",
      onclick: () => this.toggleMute() }, "\u{1F50A}");
    this.playBtn = el("button", { class: "gkh3mml-btn gkh3mml-sm",
      onclick: () => {
  ...
  buildTimeline() {
    this.ruler = el("div", { class: "gkh3mml-tmruler" });
    const ticks = 8;
    for (let i = 0; i <= ticks; i++) {
      this.ruler.append(el("span", { class: "gkh3mml-tmtick",
        style: { left: `${(i / ticks) * 100}%` } },
        fmt(this.dur * (i / ticks))));
    }
    this.selEl = el("div", { class: "gkh3mml-tmsel" });
    this.hStart = el("div", { class: "gkh3mml-tmhandle s",
      title: "Drag to move the start of the kept range",
      onmousedown: (e) => this.handleDown(e, "s") });
    this.hEnd = el("div", { class: "gkh3mml-tmhandle e",
      title: "Drag to move the end of the kept range",
      onmousedown: (e) => this.handleDown(e, "e") });
    this.playhead = el("div", { class: "gkh3mml-tmplayhead" });
    this.playTime = el("span", { class: "gkh3mml-tmplaytime" });
    this.outside = el("span", { class: "gkh3mml-tmoutside" });
    this.note = el("div", { class: "gkh3mml-tmnote" });
    this.bar = el("div", { class: "gkh3mml-tmbar",
      onmousedown: (e) => this.barDown(e) },
      this.selEl, this.hStart, this.hEnd, this.playhead);
    if (this.item.kind === "audio") {
      this.wave = el("canvas", { class: "gkh3mml-tmwave", width: 560, height: 46 });
      this.drawWave(this.wave);
    }
    const num = (label, get, set) => {
      const input = el("input", { class: "gkh3mml-tmnum", type: "text",
    ...
    this.readout = el("span", { class: "gkh3mml-tmreadout" });
    this.layoutTimeline();
    return el("div", { class: "gkh3mml-tmtimeline" },
      this.wave || null, this.ruler, this.bar,
      el("div", { class: "gkh3mml-tmnow" },
        this.outside,
        el("span", { class: "gkh3mml-tmspace" }),
  ...
  buildCrop() {
    ...
    this.cropRect = el("div", { class: "gkh3mml-tmcrop",
      onmousedown: (e) => this.cropDown(e, "move") },
      ...["nw", "ne", "sw", "se"].map((c) =>
        el("div", { class: `gkh3mml-tmcorner ${c}`,
          onmousedown: (e) => { e.stopPropagation(); this.cropDown(e, c); } })));
    this.cropBox = el("div", { class: "gkh3mml-cropbox" }, this.cropRect);
    this.cropWrap = el("div", { class: "gkh3mml-tmcropwrap" }, this.cropBox);
    requestAnimationFrame(() => {
      this.stopFit = fitToMedia(this.media, this.cropBox,
                                this.item.width, this.item.height,
                                this.stageEl);
    });
    this.cropInfo = el("span", { class: "gkh3mml-tmcropinfo" });
    this.rotBtn = el("button", { class: "gkh3mml-btn gkh3mml-sm",
    ...
    this.mirrorBtn = el("button", { class: "gkh3mml-btn gkh3mml-sm",
      title: "Flip the clip left-to-right before it's sent",
      onclick: () => {
        this.mirror = !this.mirror;
        this.syncMirror();
      } }, "\u21c4 Mirror");
    this.cropBtn = el("button", { class: "gkh3mml-btn gkh3mml-sm",
    ...
    this.aspectEl = el("select", { class: "gkh3mml-tmaspect",
    ...
    this.sizeEl = (!this.opts.refmod && (this.isStill || this.item.kind === "video"))
      ? el("select", { class: "gkh3mml-tmaspect",
    ...
    this.bakeBtn = (this.isStill && !this.opts.refmod)
      ? el("button", { class: "gkh3mml-btn gkh3mml-sm",
    ...
    this.lockEl = locked
      ...
      : null;
    return el("span", { class: "gkh3mml-tmcropbar" },
  ...
  async bake() {
    ...
    try {
      const resp = await postApi("/genkai/h3_media/bake", {
   ...
   *  not be undone. The thumbnail path already knew this (see .gkh3mml-pic.turned)
  ...
  sizeMedia() {
    ...
    const stage = m.closest(".gkh3mml-tmstage");
  ...
  async useAudio() {
    ...
    panel.render();
    try {
      const resp = await postApi("/genkai/h3_media/extract_audio", {
        headers: { "Content-Type": "application/json" },
  ...
  build() {
    ...
    const stage = (isVid || isStill)
      ? el("div", { class: "gkh3mml-tmstage" }, this.media,
    ...
    const chips = [2, 3].map((secs) =>
      this.dur > secs ? el("button", { class: "gkh3mml-btn gkh3mml-sm",
    ...
    this.overlay = el("div", { class: "gkh3mml-tmover",
      onmousedown: (e) => { if (e.target === this.overlay) this.close(); } },
      el("div", { class: "gkh3mml-tmmodal" + (isVid || still ? "" : " audio") },
        el("div", { class: "gkh3mml-tmhead" },
          el("span", { class: "gkh3mml-tmtitle" },
            `${still ? "\u25a3" : "\u2702"} ${this.item.name}`),
          (isVid || still) ? this.cropUI : null,
          el("button", { class: "gkh3mml-x", onclick: () => this.close() }, "\u2715")),
        stage,
        still ? null : this.buildTimeline(),
        still ? null : el("div", { class: "gkh3mml-tmfoot" },
          el("button", { class: "gkh3mml-btn gkh3mml-sm", title: "Previous frame (\u2190)",
            onclick: () => this.seek((this.media?.currentTime || 0) -
              1 / (this.item.fps || TRIM_FPS)) }, "\u25c0|"),
          this.playBtn,
          this.muteBtn,
          el("button", { class: "gkh3mml-btn gkh3mml-sm", title: "Next frame (\u2192)",
            onclick: () => this.seek((this.media?.currentTime || 0) +
              1 / (this.item.fps || TRIM_FPS)) }, "|\u25b6"),
          el("span", { class: "gkh3mml-tmgap" }),
          el("button", { class: "gkh3mml-btn gkh3mml-sm",
            title: "Set start to the playhead  ( [ )",
            onclick: () => { this.start =
              Math.min(this.media?.currentTime || 0, this.end - 0.1);
              this.layoutTimeline(); } }, "\u21e4 start"),
          this.numStart, el("span", { class: "gkh3mml-tmdash" }, "\u2013"),
          this.numEnd,
          el("button", { class: "gkh3mml-btn gkh3mml-sm",
          ...
          el("button", { class: "gkh3mml-btn gkh3mml-sm",
            ...
            "Last \u23ed")),
        el("div", { class: "gkh3mml-tmfoot act" },
          ...(still ? [] : chips),
          (isVid && !still && !this.opts.noAdd) ? el("button", { class: "gkh3mml-btn gkh3mml-sm",
  ...
  if (!item.width) {
    media.addEventListener(item.kind === "video" ? "loadedmetadata" : "load",
      () => {
        ...
        const cap = overlay.querySelector(".gkh3mml-lightdims");
        if (cap) cap.textContent = dimsLabel(w, h);
      });
  }
  const overlay = el("div", { class: "gkh3mml-light",
    onclick: (e) => { if (e.target === overlay) overlay.remove(); } },
    el("div", { class: "gkh3mml-lightbox" }, media,
      el("div", { class: "gkh3mml-lightcap" },
        el("span", { class: `gkh3mml-tag ${tag.startsWith("<Video") ? "vid" : "pic"}` }, tag),
        el("span", {}, item.name),
        el("span", { class: "gkh3mml-lightdims" },
          dimsLabel(item.width, item.height)),
        el("button", { class: "gkh3mml-btn", style: { marginLeft: "auto" },
 ...
 *  touch. That is right for the thumbnail path, where `.gkh3mml-cropfit` carries
...
function miniPlayer(url) {
  const fill = el("i");
  const bar = el("div", { class: "gkh3mml-bar" }, fill);
  const time = el("span", { class: "gkh3mml-time" }, "0:00");
  const btn = el("button", { class: "gkh3mml-play", title: "Play" }, "\u25b6");
  let audio = null;
...
function capabilities() {
  if (!capsPromise) {
    capsPromise = api.fetchApi("/genkai/h3_media/capabilities")
      .then((r) => r.json())
...
const TOKEN_HEADER = "X-Genkai-H3-Token";
let tokenPromise = null;
function sessionToken(fresh = false) {
  if (fresh || !tokenPromise) {
    tokenPromise = api.fetchApi("/genkai/h3_media/token")
...
async function presetApi(path, body) {
  const resp = body
    ? await postApi("/genkai/h3_media/presets" + path, {
        body: JSON.stringify(body),
        headers: { "Content-Type": "application/json" } })
    : await api.fetchApi("/genkai/h3_media/presets" + path);
...
async function uploadFile(file) {
  ...
  const resp = await postApi("/genkai/h3_media/upload", { body });
  const data = await resp.json().catch(() => ({}));
...
class LoaderPanel {
  ...
  constructor(node, opts = {}) {
    ...
    this.root = el("div", { class: "gkh3mml-panel" });
    this.root.addEventListener("click", e => {
      if (performance.now() < (this._ignoreSlotClickUntil || 0)) {
        e.preventDefault(); e.stopImmediatePropagation();
      }
    }, true);
    ...
    this.root.addEventListener("mousedown", (e) => {
      if (!e.target.closest(".gkh3mml-scalewrap")) this.closeScaleMenu();
      if (!e.target.closest(".gkh3mml-presetwrap")) this.closePresetMenu();
    });
    // Dragging a slider must not be treated as a click elsewhere.
    this.root.addEventListener("click", (e) => {
      if (e.target.closest(".gkh3mml-scalemenu")) e.stopPropagation();
    });
    this.picker = el("input", {
      type: "file", multiple: true, style: { display: "none" },
      accept: "image/*,video/*,audio/*",
      onchange: (e) => { const slot = this.pickerSlot; this.pickerSlot = null; this.add([...e.target.files], slot); e.target.value = ""; },
    });
   ...
   *  Set both, then Apply. */
  styleControl() {
    const choices = [["original", "Original"], ["gold", "Gold"], ["obsidian", "Obsidian"],
      ["studio", "Light Studio"], ["contrast", "High Contrast"]];
    const select = el("select", { class: "gkh3mml-style", title: "Panel style",
      "aria-label": "Style", onchange: (e) => {
        layoutPrefs(this.node).style = e.target.value;
        (this.node._mmlPanels || [this]).forEach(p => p.applyLayout());
        this.node.graph?.change?.();
      } }, choices.map(([value, label]) => el("option", { value }, label)));
    select.value = layoutPrefs(this.node).style;
    return el("label", { class: "gkh3mml-stylewrap" }, "Style", select);
  }

  applyLayout() {
    const prefs = layoutPrefs(this.node);
    this.root.dataset.style = prefs.style;
    this.root.style.setProperty("--gkh3mml-left", String(prefs.split));
    this.root.style.setProperty("--gkh3mml-right", String(1 - prefs.split));
    const select = this.root.querySelector(".gkh3mml-style");
    if (select) select.value = prefs.style;
    const divider = this.root.querySelector(".gkh3mml-divider");
    divider?.setAttribute("aria-valuenow", String(Math.round(prefs.split * 100)));
  }

  divider() {
    const divider = el("div", { class: "gkh3mml-divider", role: "separator", tabIndex: 0,
      title: "Drag to resize columns", "aria-label": "Column width",
      "aria-orientation": "vertical", "aria-valuemin": "25", "aria-valuemax": "80" });
    const update = (ratio) => {
      layoutPrefs(this.node).split = Math.max(.25, Math.min(.8, ratio));
      (this.node._mmlPanels || [this]).forEach(p => p.applyLayout());
    };
    let dragging = false;
    ...
    divider.addEventListener("pointermove", e => {
      ...
      update((e.clientX - r.left) / r.width);
    });
    const finish = () => {
      if (!dragging) return;
      dragging = false; divider.classList.remove("active"); this.node.graph?.change?.();
    };
    ...
    divider.addEventListener("keydown", e => {
      ...
      e.preventDefault(); e.stopPropagation();
      update(layoutPrefs(this.node).split + (e.key === "ArrowLeft" ? -.025 : .025));
      this.node.graph?.change?.();
    });
    divider.addEventListener("dblclick", () => { update(.5); this.node.graph?.change?.(); });
    return divider;
  }
  ...
  scaleControl() {
    ...
    const slider = (key, label) => {
      ...
      const out = el("input", { type: "number", class: "gkh3mml-scaleval",
      ...
      const input = el("input", { type: "range", class: "gkh3mml-scalerange",
        min: String(Math.round(SCALE_MIN * 100)),
        max: String(Math.round(maxFor(key) * 100)), step: "5",
        value: String(Math.round(pending[key] * 100)),
        oninput: (e) => {
      ...
      inputs[key] = input;
      outs[key] = out;
      return el("label", { class: "gkh3mml-scalerow" },
        el("span", { class: "gkh3mml-scalelabel" }, label), input, out,
        el("span", { class: "gkh3mml-scalepct" }, "%"));
    };
    ...
    const applyBtn = el("button", { class: "gkh3mml-btn gkh3mml-sm",
      onclick: (e) => { e.stopPropagation(); commit(pending.node, pending.text); } },
      "Apply");

    const menu = el("div", { class: "gkh3mml-scalemenu" },
      slider("node", "Node size"),
      slider("text", "Text size"),
      el("label", { class: "gkh3mml-resizecheck" },
        el("input", { type: "checkbox", checked: layoutPrefs(this.node).dynamicResize,
          onchange: e => {
            layoutPrefs(this.node).dynamicResize = e.target.checked;
            if (e.target.checked) responsiveSizing(this.node);
            else applyNodeSize(this.node, prefs.node);
            this.node.graph?.change?.(); this.node.setDirtyCanvas?.(true, true);
          } }), "Resize contents with node"),
      el("div", { class: "gkh3mml-scalefoot" },
        el("span", {}, "Remembered for new nodes"),
        el("button", { class: "gkh3mml-btn gkh3mml-sm",
          onclick: (e) => { e.stopPropagation(); commit(1, 1); } }, "Reset"),
        applyBtn));

    const btn = el("button", { class: "gkh3mml-btn gkh3mml-sm",
      title: "Node and text size",
      onclick: (e) => {
        e.stopPropagation();
        const open = menu.classList.toggle("on");
        btn.classList.toggle("on", open);
        if (open) setTimeout(() => outs.node.focus(), 0);
      } }, "\u2921 Size");
    this._scaleMenu = menu;
    this._scaleBtn = btn;
    return el("span", { class: "gkh3mml-scalewrap" }, btn, menu);
  }
  ...
  presetPicker() {
    const list = el("div", { class: "gkh3mml-presetlist" });
    ...
    const filter = el("input", { type: "text", class: "gkh3mml-presetfilter",
    ...
    const catSel = el("select", { class: "gkh3mml-presetcatfilter",
    ...
    const catBtn = el("button", { class: "gkh3mml-btn gkh3mml-sm gkh3mml-presetcatedit",
    ...
    const bar = el("div", { class: "gkh3mml-presetbar" }, filter, catSel, catBtn);
    const renameRow = el("div", { class: "gkh3mml-presetrenamerow" });
    ...
    const paintRename = () => {
      ...
      const input = el("input", { type: "text", class: "gkh3mml-presetcatnew",
      ...
      renameRow.replaceChildren(input,
        ...
        el("button", { class: "gkh3mml-btn gkh3mml-sm gkh3mml-danger",
    ...
    const paint = () => {
      ...
      if (!hits.length) {
        list.replaceChildren(el("div", { class: "gkh3mml-presetempty" },
    ...
    const menu = el("div", { class: "gkh3mml-presetmenu" }, bar, renameRow, list);
    const btn = el("button", { class: "gkh3mml-presetbtn",
    ...
    this._presetBtn = btn;
    return el("div", { class: "gkh3mml-presetwrap" }, btn, menu);
  }
  ...
  catEditor(p, paint) {
    ...
    const fresh = el("input", { type: "text", class: "gkh3mml-presetcatnew",
    ...
    const sel = el("select", { class: "gkh3mml-presetcat",
      onmousedown: (e) => e.stopPropagation(),
      onclick: (e) => e.stopPropagation(),
      onchange: () => {
    ...
    return el("div", { class: "gkh3mml-presetitem editing",
      onmousedown: (e) => e.stopPropagation(),
      onclick: (e) => e.stopPropagation() },
      el("span", { class: "gkh3mml-presetitemname" }, p.name),
      sel, fresh,
      el("button", { class: "gkh3mml-btn gkh3mml-sm", onclick: commit }, "Set"),
      el("button", { class: "gkh3mml-btn gkh3mml-sm",
  ...
  trimBtn(item) {
    ...
    return el(still ? "button" : "span", {
      class: "gkh3mml-trimbtn" + (active ? " on" : ""),
  ...
  powerBtn(item) {
    ...
    if (item.kind === "picture") return el("button", {
      ...
      role: "switch", "aria-checked": String(on), "aria-label": "Enable picture",
      title: on ? "Disable picture — keep it in this slot" : "Enable picture",
      onclick: e => { e.stopPropagation(); this.toggle(item); },
    }, el("span", { class: "gkh3mml-switch-knob" }));
    return el("span", {
      class: "gkh3mml-power" + (on ? " on" : ""),
  ...
  bindSlot(element, position) {
    ...
    element.addEventListener("dragover", e => {
      if (!e.dataTransfer?.types?.some(t => t === SLOT_MIME || t === "Files")) return;
      e.preventDefault(); e.stopPropagation(); element.classList.add("over");
    });
    element.addEventListener("dragleave", () => element.classList.remove("over"));
    element.addEventListener("drop", e => {
      const data = e.dataTransfer;
      if (!data?.types?.some(t => t === SLOT_MIME || t === "Files")) return;
      e.preventDefault(); e.stopPropagation();
      element.classList.remove("over"); this.root.classList.remove("drop");
      if (this.busy) return;
  ...
  reorderable(element, item) {
    element.draggable = false;
    element.dataset.uid = item.uid;
    element.dataset.kind = item.kind;
    // Prevent the browser's native image drag from taking over the slot.
    ...
    element.addEventListener("lostpointercapture", e => finish(e, true));
    ...
    return this.bindSlot(element, item.genkai_slot);
  }

  emptySlot(kind, index) {
    const position = (kind === "picture" ? 0 : kind === "video" ? 9 : 12) + index - 1;
    const slot = el("div", { class: "gkh3mml-slot",
      title: "Empty slot — drop a reference or hover and press Ctrl+V",
      onclick: () => { this.pickerSlot = position; this.picker.click(); } },
      el("span", {}, `${kind} ${index}`));
    return this.bindSlot(slot, position);
  }
  ...
  render() {
    try {
      ...
      console.error("[H3 Media Loader (genkai)] render failed:", err);
    }
  }
  ...
  drawPanel() {
    ...
    kids.push(el("div", { class: "gkh3mml-top" },
      ...
      el("span", { class: "gkh3mml-topspace" }),
      this.styleControl(),
      this.scaleControl(),
      ...
      this.items.length
        ? el("button", { class: "gkh3mml-btn gkh3mml-sm",
            ...
            "Unload media")
        : null,
      el("span", { class: "gkh3mml-count" + (total > MAX.total ? " over" : "") },
        `${total} / ${MAX.total}`),
      el("span", { class: "gkh3mml-count" + (audioCount(this.items) > MAX.audio ? " over" : ""),
    ...
    if (this.presetPrompt === "save") {
      ...
      const catNew = el("input", { type: "text", class: "gkh3mml-presetcatnew",
        placeholder: "new category", style: { display: "none" } });
      const cat = el("select", { class: "gkh3mml-presetcat",
        onchange: () => {
      ...
      kids.push(el("div", { class: "gkh3mml-presetrow" },
        el("span", { class: "gkh3mml-presetlbl" }, "save as"), input, cat, catNew,
        el("button", { class: "gkh3mml-btn gkh3mml-sm", onclick: go }, "Save"),
        el("button", { class: "gkh3mml-btn gkh3mml-sm",
      ...
      kids.push(el("div", { class: "gkh3mml-presetrow" },
        el("span", { class: "gkh3mml-presetlbl" }, "preset"),
        select,
        el("button", { class: "gkh3mml-btn gkh3mml-sm", title: "Save the current set",
          onclick: () => { this.presetPrompt = "save"; this.render(); } }, "Save"),
        el("button", { class: "gkh3mml-btn gkh3mml-sm", title: "Delete the selected preset",
          onclick: () => {
    ...
    pics.forEach((it) => {
      ...
      picCells.push(this.reorderable(el("div",
        { class: "gkh3mml-slot filled pic" + (isOn(it) ? "" : " off") },
        (() => {
          // Badge and img are SIBLINGS in the slot: .gkh3mml-pic is absolutely
          ...
          const badge = el("span", { class: "gkh3mml-dims" + (it.crop ? " cut" : "") },
            dimsLabel(ow, oh));
          ...
          let marquee = null;
          if (it.crop) {
            const box = el("div", { class: "gkh3mml-cropbox" },
              el("div", { class: "gkh3mml-cropmark", style: {
                left: `${(it.crop.x ?? 0) * 100}%`,
                top: `${(it.crop.y ?? 0) * 100}%`,
                width: `${(it.crop.w ?? 1) * 100}%`,
                height: `${(it.crop.h ?? 1) * 100}%`,
              } }));
            marquee = el("div", { class: "gkh3mml-cropfit", style: flip }, box);
    ...
    right.append(el("div", { class: "gkh3mml-sec" }, "videos",
      el("button", { class: "gkh3mml-helpbtn",
    ...
    vids.forEach((it) => {
      ...
      const vcell = el("div", { class: "gkh3mml-slot filled vid" + (isOn(it) ? "" : " off") },
        row);
    ...
    right.append(el("div", { class: "gkh3mml-sec" }, "standalone audio",
    ...
    auds.forEach((it) => {
      ...
      const arow = el("div", { class: "gkh3mml-row" },
          this.powerBtn(it),
          player.btn,
          el("div", { class: "gkh3mml-meta", style: { flex: "0 0 auto", maxWidth: "38%" } },
            el("div", { class: "gkh3mml-tag aud" },
              isOn(it) ? (tags.get(it) || "").slice(1, -1) : "off"),
            el("div", { class: "gkh3mml-name", title: it.name }, it.name)),
          ...
          el("span", { class: "gkh3mml-x", title: "Remove",
            onclick: () => this.remove(it) }, "\u2715"));
      const acell = el("div",
        { class: "gkh3mml-slot filled aud" + (isOn(it) ? "" : " off") },
        arow);
...
function splitHelp(anchor) {
  const rows = SPLIT_HELP.map(([mode, body]) =>
    el("div", { class: "gkh3mml-helprow" },
      el("span", { class: `gkh3mml-helpmode ${mode}` }, mode),
      el("p", {}, body)));

  const wiring = SPLIT_WIRING.map(([mode, out, native, tags]) =>
    el("div", { class: "gkh3mml-wirerow" },
      el("span", { class: `gkh3mml-helpmode ${mode}` }, mode),
      el("code", {}, out), el("span", { class: "gkh3mml-arrow" }, "\u2192"),
      el("code", {}, native),
      el("span", { class: "gkh3mml-tags" }, tags)));

  const box = el("div", { class: "gkh3mml-help" },
    el("div", { class: "gkh3mml-helphead" }, "split audio",
      el("button", { title: "Close", onclick: () => close() }, "\u2715")),
    el("div", { class: "gkh3mml-helpbody" },
      rows,
      el("div", { class: "gkh3mml-helpsub" }, "where the track comes out"),
      wiring,
      el("p", { class: "gkh3mml-helpnote" },
...
function flash(text) {
  const t = el("div", { class: "gkh3mml-toast" }, text);
...
export function openLoaderModal(node, opts = {}) {
  ...
  const close = () => {
    ...
    try { onClose?.(); } catch (e) {
      console.error("[H3 Media Loader (genkai)] close callback failed:", e);
    }
  };
  const esc = (e) => { if (e.key === "Escape") close(); };
  const overlay = el("div", { class: "gkh3mml-overlay",
  ...
  window.addEventListener("keydown", esc);
  document.body.append(overlay);
  scaleOverlay(node, [[overlay.querySelector(".gkh3mml-modal"), 1140, 780]]);
  return panel;
}
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      try {
        ...
        this._mmlNativeCompute = widget.computeSize;
        ...
        console.error("[H3 Media Loader (genkai)] setup failed for this node:", err);
        try { this.addWidget("button", "\u26a0 UI failed \u2014 click", null, () => {
          alert("H3 Media Loader (genkai) could not build its interface.\n\n" + err +
    ...
    nodeType.prototype.onResize = function (size) {
      try {
        if (layoutPrefs(this).dynamicResize) {
          responsiveSizing(this);
          size[0] = Math.max(NODE_W, size[0]); size[1] = Math.max(470, size[1]);

...
| Нода | Что делает |
| --- | --- |
| **PromptSync (genkai)** | Показывает видео рядом с исходным промптом, подсвечивает текущий временной отрезок и отображает звуковую волну. |
| **PromptSync + Save (genkai)** | Совмещает просмотрщик PromptSync с сохранением итогового MP4 со звуком и встроенными метаданными. |
| **Folder Search (genkai)** | Находит файлы по маске, выдаёт весь список или по одному файлу за запуск. |
| **Image Expand With Fill (genkai)** | Добавляет поля вокруг изображения с заполнением крайними пикселями или чёрным цветом. |

...
Перезапустите ComfyUI и обновите страницу браузера через **Ctrl+F5**. Ноды доступны через поиск по названию; просмотрщики находятся в `GENKAI/Video`, медианоды H3 — в `GENKAI/Media`, Seed Slots — в `GENKAI/Seeds`, Folder Search и Image Expand With Fill — в `GENKAI nodes`.
